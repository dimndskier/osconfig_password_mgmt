  ...
  case "${os_brand}-${os_revision}" {  # Start CASE 1
    'redhat-6': {
      exec {'alter_grub1_password':
        command => "/bin/mv -f /boot/grub/grub.conf.backup-$grubchanged    /boot/grub/grub.conf",
...
#
    'redhat-7': {
      exec {'remove_united_grub2_config':
        command => "/bin/mv -f /boot/grub2/grub.cfg.backup-$grubchanged    /boot/grub2/grub.cfg",
        path    => "/usr/bin:/bin:/usr/sbin:/sbin",
